...
exports.pluginVersionCheck = void 0;
...
const makeDynamicPermalinkFn_1 = __importDefault(require("../routes/makeDynamicPermalinkFn"));
const pluginVersionCheck = (elderVersion, pluginVersion) => {
    ...
    if (eMajor > pMajor)
        enabled = true;
    if (eMajor === pMajor && eMinor > pMinor)
        enabled = true;
    if (eMajor === pMajor && eMinor === pMinor && ePatch >= pPatch)
        enabled = true;
    return enabled;
};
exports.pluginVersionCheck = pluginVersionCheck;
async function plugins(elder) {
    /**
    ...
    for (let i = 0; i < pluginNames.length; i += 1) {
        ...
        const srcPlugin = path_1.default.resolve(elder.settings.srcDir, pluginPath);
        ...
        if (!plugin) {
            const pkgPath = path_1.default.resolve(elder.settings.rootDir, './node_modules/', pluginName);
            if (fs_extra_1.default.existsSync(pkgPath)) {
                usesNodeModulesFolder = true;
                // eslint-disable-next-line import/no-dynamic-require
                const pluginPackageJson = require(path_1.default.resolve(pkgPath, './package.json'));
                const pluginPkgPath = path_1.default.resolve(pkgPath, pluginPackageJson.main);
                // eslint-disable-next-line import/no-dynamic-require
        ...
        if (typeof plugin.init === 'function' || (plugin.init && typeof plugin.init.then === 'function')) {
            plugin =
                ...
                (await plugin.init({
                    ...
                    settings: createReadOnlyProxy_1.default(elder.settings, 'Settings', 'plugin init()'),
                })) || plugin;
        }
        if (plugin.minimumElderjsVersion) {
            if (plugin.minimumElderjsVersion.split('.').length !== 3)
                console.error(`${pluginName} has a malformed minimumElderjsVersion of ${plugin.minimumElderjsVersion}. Please tell the developer to update.`);
            if (!exports.pluginVersionCheck(elder.settings.version, plugin.minimumElderjsVersion)) {
                console.error(Error(`Plugin ${pluginName} requires Elder.js version ${plugin.minimumElderjsVersion} and you are using ${elder.settings.version}. Disabling plugin. If you want to use this plugin, please update to the required version.`));
                // eslint-disable-next-line no-continue
                continue;
            }
        }
        const validatedPlugin = __1.validatePlugin(plugin);
        if (validatedPlugin) {
            ...
            pluginHooksArray.forEach((hook) => {
                const validatedHook = __1.validateHook(hook);
                if (validatedHook) {
            ...
            if (Object.hasOwnProperty.call(plugin, 'routes')) {
                ...
                for (let ii = 0; ii < routeNames.length; ii += 1) {
                    ...
                    if (typeof plugin.routes[routeName].permalink === 'undefined') {
                        console.error(`WARN: Plugin ${routeName} does not define a permalink function on it's routes. Setting default permalink to \`/${routeName}/\``);
                    ...
                    if (typeof plugin.routes[routeName].permalink === 'string') {
                        ...
                        plugin.routes[routeName].permalink = makeDynamicPermalinkFn_1.default(plugin.routes[routeName].permalink);
                        plugin.routes[routeName].$$meta = {
                            ...plugin.routes[routeName].$$meta,
                            routeString,
                    ...
                    plugin.routes[routeName].permalink = wrapPermalinkFn_1.default({
                    ...
                    if (typeof plugin.routes[routeName].template === 'string' &&
                        ...
                        const ssrComponent = path_1.default.resolve(elder.settings.$$internal.ssrComponents, `./${usesNodeModulesFolder ? 'node_modules/' : 'plugins/'}${pluginName}/${templateName}.js`);
                        const template = elder.settings.$$internal.findComponent(templateName, usesNodeModulesFolder ? 'node_modules' : 'plugins');
                        if (!template.ssr && !fs_extra_1.default.existsSync(ssrComponent)) {
                        ...
                        plugin.routes[routeName].templateComponent = __1.svelteComponent(templateName, usesNodeModulesFolder ? 'node_modules' : 'plugins');
                    }
                    else {
                        console.error(Error(`Plugin Route: ${routeName} added by plugin ${pluginName} does not have a template defined. Disabling this route.`));
                        // eslint-disable-next-line no-continue
                        continue;
                    }
                    if (typeof plugin.routes[routeName].layout === 'string' &&
                        plugin.routes[routeName].layout.endsWith('.svelte')) {
                        const layoutName = plugin.routes[routeName].layout.replace('.svelte', '');
                        const ssrComponent = path_1.default.resolve(elder.settings.$$internal.ssrComponents, `./plugins/${pluginName}/${layoutName}.js`);
                        const layout = elder.settings.$$internal.findComponent(layoutName, usesNodeModulesFolder ? 'node_modules' : 'plugins');
                        if (!layout.ssr && !fs_extra_1.default.existsSync(ssrComponent)) {
                            console.warn(`Plugin Route: ${routeName} added by plugin ${pluginName} has an error. No SSR svelte component found ${layoutName}. This may cause unexpected outcomes. If you believe this should be working, make sure rollup has run before this file is initialized. If the issue persists, please contact the plugin author. Expected location \`${ssrComponent}\``);
                        }
                        plugin.routes[routeName].layoutComponent = __1.svelteComponent(layoutName, usesNodeModulesFolder ? 'node_modules' : 'plugins');
                    }
                    else {
                        plugin.routes[routeName].layout = 'Layout.svelte';
                        const ssrComponent = path_1.default.resolve(elder.settings.$$internal.ssrComponents, `./layouts/Layout.js`);
                        if (!fs_extra_1.default.existsSync(ssrComponent)) {
                            console.error(`Plugin Route: ${routeName} added by plugin ${pluginName} requires a /src/layouts/Layout.svelte to be compiled at ${ssrComponent}. Disabling this route.`);
                            // eslint-disable-next-line no-continue
                            continue;
                        }
                        plugin.routes[routeName].layoutComponent = __1.svelteComponent('Layout.svelte', 'layouts');
                    }
                    // eslint-disable-next-line @typescript-eslint/no-unused-vars
