...
exports.getWorkerCounts = void 0;
...
async function build(initializationOptions = {}) {
    try {
        const settings = getConfig_1.default({ ...initializationOptions, context: 'build' });
        if (!settings.build)
            throw new Error('Is not a build. (Should never happen, for TS)');
        ...
        if (cluster_1.default.isMaster) {
            ...
            const mElder = new Elder_1.Elder({ ...initializationOptions, context: 'build' });
                                    ...
                                    Object.keys(counts).forEach((id) => {
                                        if (counts[id].bar) {
                                            counts[id].bar.update(counts[id].count, {
                                                ...
                                                avgRequestTime: `${Math.round(((Date.now() - counts[id].startTime) / counts[id].count) * 100) / 100}ms`,
                                            });
                                        }
                                    });
                                }
            ...
            if (time > 60000) {
                console.log(`Build ${success ? 'Completed Successfully:' : 'Failed:'} Built ${totalRequests} pages in ${Math.round((time / 60000) * 100) / 100} minutes`);
            }
            else if (time > 1000) {
                console.log(`Build ${success ? 'Completed Successfully:' : 'Failed:'} Built ${totalRequests} pages in ${Math.round((time / 1000) * 100) / 100} seconds`);
            }
            else {
                console.log(`Build ${success ? 'Completed Successfully:' : 'Failed:'} Built ${totalRequests} pages in ${time}ms`);
            }
            ...
            process.on('message', async (msg) => {
                if (msg.cmd === 'start') {
                    const wElder = new Elder_1.Elder({ ...initializationOptions, context: 'build', worker: true });
