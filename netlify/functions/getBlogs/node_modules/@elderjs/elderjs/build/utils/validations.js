...
exports.shortcodeSchema = exports.pluginSchema = exports.routeSchema = exports.hookSchema = exports.configSchema = exports.getDefaultConfig = exports.getDefaultRollup = exports.validateShortcode = exports.validateHook = exports.validatePlugin = exports.validateRoute = void 0;
const yup = __importStar(require("yup"));
const hookInterface_1 = __importDefault(require("../hooks/hookInterface"));
/**
 * Important note:
...
exports.shortcodeSchema = shortcodeSchema;
const configSchema = yup.object({
    ...
    srcDir: yup
        ...
        .notRequired()
        .default('src')
        .label("Where Elder.js should find it's expected file structure. If you are using a build step such as typescript on your project, you may need to edit this. "),
    props: yup.object({
        hydration: yup
            .string()
            .notRequired()
            .default('hybrid')
            .label('How should props be handled. "hybrid": writes props under 2kb to the html, props > 2kb are an external file. "html": props are written to page html. "file" writes props to a hashed external file.'),
    ...
    css: yup
        ...
        .required()
        .default('file')
        .test('inline-lazy-none-file', 'Css in your elder.config.js must be "inline", "file", "lazy", or "none".', (value) => ['file', 'inline', 'lazy', 'none'].includes(value))
        .label(`How should css found in svelte files be handled? 'inline' emits styles into the head. 'file' adds a file include into the head. 'none' doesn't do anything with the styles.`),
});
...
const routeSchema = yup.object({
    ...
    all: yup
        .mixed()
        .required()
        .test('isFunction', 'all() should be a function or async function or an array', (value) => typeof value === 'function' || Array.isArray(value) || (typeof value === 'object' && value.then === 'function'))
    ...
    name: yup
        ...
        .optional()
        .min(1)
        .label('A name used to identify the route internally. If not defined, it uses [routeName] from "/routes/[routeName]/route.js". Mainly used with helpers.permalink.[name]().'),
    dynamic: yup
        .boolean()
        .optional()
        .default(false)
        .label('Enables dynamic route parameters in SSR mode. This means /:foo/ will be derived from the visited URL. Useful for logged in experiences.'),
});
...
const pluginSchema = yup.object({
    ...
    init: yup
        ...
        .test('isFunction', 'Init should be a function or async function', (value) => typeof value === 'function' || (typeof value === 'object' && value.then === 'function')),
...
function validatePlugin(plugin) {
    try {
        ...
        console.error(`Plugin ${(plugin && plugin.$$meta && plugin.$$meta.addedBy) || ''} does not have the required fields. Please let the author know`, err.errors, err.value);
        return false;
    }
}
...
function validateShortcode(shortcode) {
    try {
        ...
        if (shortcode && shortcode.$$meta && shortcode.$$meta.type === 'plugin') {
            console.error(`Plugin ${shortcode.$$meta.addedBy} uses a shortcode, but it is ignored due to error(s). Please create a ticket with that plugin so the author can investigate it.`, err.errors, err.value);
        }
        else {
            console.error(`Hook ignored due to error(s).`, err.errors, err.value);
        }
