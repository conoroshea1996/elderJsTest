...
exports.preprocessSvelteContent = void 0;
const inlineSvelteComponent_1 = require("./inlineSvelteComponent");
...
const createReplacementString = ({ input, name, props }) => {
    ...
    return inlineSvelteComponent_1.inlinePreprocessedSvelteComponent({ name, props, options });
};
const preprocessSvelteContent = (content) => {
    ...
    if (wrappedComponents && wrappedComponents.length > 0) {
        throw new Error(`Elder.js only supports self-closing syntax on hydrated components. This means <Foo /> not <Foo></Foo> or <Foo>Something</Foo>. Offending component: ${wrappedComponents[0][0]}. Slots and child components aren't supported during hydration as it would result in huge HTML payloads. If you need this functionality try wrapping the offending component in a parent component without slots or child components and hydrate the parent component.`);
    }
    return output;
};
exports.preprocessSvelteContent = preprocessSvelteContent;
const partialHydration = {
    markup: async ({ content }) => {
        return { code: exports.preprocessSvelteContent(content) };
    },
};
